...
-- ejecutan como una única unidad lógica de trabajo. Para 
-- garantizar la integridad de los datos, toda transacción 
...
telefono, fax, linea_direccion1, ciudad, codigo_empleado_rep_ventas, limite_credito) 
VALUES (52, 'Jardines S.A.', 'Juan', 'Pérez', '600112233', '600112234', 
'Calle Falsa 123', 'Madrid', 11, 5000);
...
VALUES (52, 'PayPal', 'PAY-999', CURDATE(), 100.00);

-- Creamos el segundo punto de control: El pago también está "seguro" 
SAVEPOINT pago_registrado;

-- 3. Intentamos añadir un producto de regalo (un detalle de pedido)
-- Supongamos que intentamos regalar un producto que NO existe (error de FK)
-- o que simplemente queremos probar si falla. 
INSERT INTO detalle_pedido (codigo_pedido, codigo_producto, 
cantidad, precio_unidad, numero_linea) 
VALUES (999, 'PRODUCTO_INEXISTENTE', 1, 0.00, 1);

-- Si la inserción anterior falla o decidimos no dar el regalo:
-- VOLVEMOS AL PUNTO DEL PAGO, descartando solo el intento del regalo. 
...
call insertar_pedido_seguro();

call insertar_pedido_seguro(5, 129, "FR-101", 5, 0);

call insertar_pedido_seguro(10, 10, "FR-101", 5, 13);